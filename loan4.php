<?php
session_start();
include 'db.php'; // Ensure db.php connects and returns $conn
$conn = connectDB();
...
/**
 * Function to calculate next due date and payment details, adjusting for advance payments
 * based on principal reduction.
 */
function calculateNextPaymentDetails($conn, $loanInfo, $lrInfo)
{
    $details = [
        'nextDueDate' => 'N/A',
        'nextPaymentAmount' => 0.00,
        'projectedRemainingBalance' => 0.00,
        'isDue' => false,
        'remainingPayments' => 0,
    ];

    // Ensure both active loan and loan request data exist
    if (!$loanInfo || !$lrInfo) {
        return $details;
    }

    $remainingLoan = floatval($loanInfo['loan_amount']);
    $loanStartDate = $loanInfo['created_at'];
    ...
    // 1. Determine Loan Start Date
    try {
        $startDate = new DateTime($loanStartDate);
    } catch (Exception $e) {
        return $details;
    }

    // 2. Calculate Total Payments and Per-Payment Amount
    switch ($frequency) {
        case 'monthly':
    ...
    $details['nextPaymentAmount'] = round($paymentAmount, 2);

    // --- ADVANCE PAYMENT ADJUSTMENT ---

    // Calculate the total amount of principal that has been paid off
    $paidPrincipal = $totalLoanAmount - $remainingLoan;

    // Estimate the number of full payments covered by the amount paid.
    // We use floor() to count only completed installments.
    if ($paymentAmount > 0) {
        $estimatedPaymentsPaid = floor($paidPrincipal / $paymentAmount);
    } else {
        $estimatedPaymentsPaid = 0;
    }

    // --- END ADVANCE PAYMENT ADJUSTMENT ---

    // 3. Find the Next Due Date

    $currentDate = new DateTime();
    $nextDueDate = clone $startDate;

    // Start the loop from the installment *after* the estimated payments made
    for ($i = $estimatedPaymentsPaid + 1; $i <= $totalPayments; $i++) {
        $nextDueDate = clone $startDate;

        if ($frequency === 'monthly') {
            // Calculate i-th month from start date
            $nextDueDate->modify("+$i months");
            $dayOfMonth = $startDate->format('j');

            // Handle end-of-month dates (e.g., loan starts 31st Jan, 1st due date is 28th Feb)
            if ($dayOfMonth > 28 && $nextDueDate->format('j') != $dayOfMonth) {
                $nextDueDate->modify('last day of previous month');
            }

        } else {
            $nextDueDate->modify("+$i $interval");
        }

        // The first calculated date ($nextDueDate) that is in the future is the one to show.
        if ($nextDueDate > $currentDate) {
            break;
        }
    }

    // Check if the contractual term has fully ended based on the calculation
    if ($estimatedPaymentsPaid >= $totalPayments || $nextDueDate <= $currentDate) {
        // Check if any balance remains
        if ($remainingLoan > 0) {
            $details['nextDueDate'] = 'Past Due (Term Ended)';
            $details['nextPaymentAmount'] = round($remainingLoan, 2);
            $details['isDue'] = true;
            $details['remainingPayments'] = 1;
            $details['projectedRemainingBalance'] = 0.00;
            return $details;
        } else {
            // Fully paid
            $details['nextDueDate'] = 'Loan Completed';
            return $details;
        }
    }

    $details['nextDueDate'] = $nextDueDate->format('F j, Y');

    // 4. Calculate Payments Remaining and Projected Balance

    // Payments Remaining based on current activeloan balance
    if ($paymentAmount > 0) {
        $details['remainingPayments'] = (int) ceil($remainingLoan / $paymentAmount);
    }

    // Projected remaining balance AFTER the next required payment
    $details['projectedRemainingBalance'] = max(0.00, $remainingLoan - $details['nextPaymentAmount']);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['makePayment'])) {
    ...
    // Fetch active loan (Fetch created_at)
    $loanStmt = $conn->prepare("
            SELECT id, user_id, loan_amount, created_at
            FROM activeloan
            WHERE user_id = ?
            ORDER BY id DESC
            LIMIT 1
    ...
    // Find corresponding loanrequest (latest approved)
    $lr = $conn->prepare("
    ...
    if (!$loanReq) {
        // Fallback to latest loanrequest
        $lr2 = $conn->prepare("
                SELECT id, loan_amount, loan_term, payment_frequency
                FROM loanrequest
                WHERE user_id = ?
    ...
    try {
        ...
        if ($newLoanAmount <= 0) {
            // Fully paid -> delete from activeloan and update loanrequest status to Paid
            $delActive = $conn->prepare("DELETE FROM activeloan WHERE id = ?");
            $delActive->bind_param("i", $activeLoanId);
            $delActive->execute();

            $updateReq = $conn->prepare("UPDATE loanrequest SET status = 'Paid' WHERE id = ?");
            ...
            // Compute remaining payments (using integer ceil for display)
...
/* ======================================================
ALWAYS SHOW REMAINING BALANCE + PAYMENTS REMAINING + NEXT DUE DATE
====================================================== */
...
// Get active loan (Fetch created_at)
...
if ($loanInfo) {
    ...
    if ($lrInfo) {
        ...
        // Recalculate full payment details using the modified function
        $paymentDetails = calculateNextPaymentDetails($conn, $loanInfo, $lrInfo);

        $nextDueDate = $paymentDetails['nextDueDate'];
        $nextPaymentAmount = $paymentDetails['nextPaymentAmount'];
        $projectedRemainingBalance = $paymentDetails['projectedRemainingBalance'];
        $paymentsRemaining = $paymentDetails['remainingPayments'];
    }
}
...
$nextPaymentAmountJs = number_format((float) $nextPaymentAmount, 2, '.', '');
$projectedRemainingBalanceJs = number_format((float) $projectedRemainingBalance, 2, '.', '');

// LOAN SUBMISSION POST BLOCK (Unchanged)
...
<body>
    ...
    <script>
        ...
        // New variables for payment tracking
        const NEXT_DUE_DATE = "<?= addslashes($nextDueDate) ?>";
        const NEXT_PAYMENT_AMOUNT = <?= $nextPaymentAmountJs ?>;
        const PROJECTED_REMAINING_BALANCE = <?= $projectedRemainingBalanceJs ?>;
